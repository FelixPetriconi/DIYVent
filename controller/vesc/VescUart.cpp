...
#include "VescUart.h"
#include "../common/IOStreamHelper.hpp"
...
void VescUart::setSerialPort(const modm::Uart* port)
{
	_serialPort = port;
}
...
int VescUart::receiveUartMessage(uint8_t * payloadReceived) {
	...
	modm::Timestamp timeout = modm::Clock::now();// + std::chrono::duration_cast<std::chrono::milliseconds>(100);

	while ( modm::Clock::now() < timeout && messageRead == false) {

		uint8_t readByte{0};
		while (false/*_serialPort->read(readByte)*/) {

			messageReceived[counter++] = readByte;
			...
			if (counter == 2) {
				...
				switch (messageReceived[0])
				{
					...
					case 3:
						...
						MODM_LOG_DEBUG << "Message is larger than 256 bytes - not supported";
					break;

					default:
						MODM_LOG_DEBUG << "Unvalid start bit";
					break;
				}
			}
			...
			if (counter == endMessage && messageReceived[endMessage - 1] == 3) {
				...
				MODM_LOG_DEBUG << "End of message reached!";
				messageRead = true;
...
bool VescUart::unpackPayload(uint8_t * message, int lenMes, uint8_t * payload) {
	...
	MODM_LOG_DEBUG << "SRC received: " << crcMessage;
	...
	MODM_LOG_DEBUG << "SRC calc: " << crcPayload;

	if (crcPayload == crcMessage) {
		MODM_LOG_DEBUG << "Received: ";
		serialPrint(message, lenMes);
		MODM_LOG_DEBUG << "\n";
		MODM_LOG_DEBUG << "Payload :      ";
...
int VescUart::packSendPayload(uint8_t * payload, int lenPay) {
	...
	MODM_LOG_DEBUG << "UART package send: "; 
	serialPrint(messageSend, count);

	// Sending package
	//_serialPort->write(messageSend, count);
...
void VescUart::setNunchuckValues() {
	...
	MODM_LOG_DEBUG << "Data reached at setNunchuckValues:";
	MODM_LOG_DEBUG << "valueX = " << nunchuck.valueX << " valueY = " << nunchuck.valueY << "\n";
	MODM_LOG_DEBUG << "LowerButton = " << nunchuck.lowerButton << " UpperButton = " << nunchuck.upperButton;
...
void VescUart::printVescValues() {
	MODM_LOG_DEBUG << "avgMotorCurrent: " << data.avgMotorCurrent << "\n";
	MODM_LOG_DEBUG << "avgInputCurrent: " << data.avgInputCurrent << "\n";
	MODM_LOG_DEBUG << "dutyCycleNow: " << data.dutyCycleNow << "\n";
	MODM_LOG_DEBUG << "rpm: " << data.rpm << "\n";
	MODM_LOG_DEBUG << "inputVoltage: " << data.inpVoltage << "\n";
	MODM_LOG_DEBUG << "ampHours: " << data.ampHours << "\n";
	MODM_LOG_DEBUG << "ampHoursCharges: " << data.ampHoursCharged << "\n";
	MODM_LOG_DEBUG << "tachometer: " << data.tachometer << "\n";
	MODM_LOG_DEBUG << "tachometerAbs: " << data.tachometerAbs << "\n";
